...
def hold_on_submit(hook_config, event):
    ...
    job = event.job
    if job.queue:
        queue = pbs.get_queue(job.queue.name)
    else:
        queue = pbs.get_queue(pbs.server().default_queue.name)
    # note: Resource_List does not act like a python dict
    # "random key" in Resource_List == TRUE!!!
    # must do `Resource_List["random key"] is None` instead
    if job.Resource_List["skipcyclesubhook"] is not None:
        if job.Resource_List["skipcyclesubhook"]:
            debug("skipcyclesubhook is true for job %s, skipping" % job.id)
            return
    elif (
        queue.resources_default
        and "skipcyclesubhook" in queue.resources_default
        and queue.resources_default["skipcyclesubhook"]
    ):
        debug("skipcyclesubhook is true for job %s, skipping" % job.id)
        return
    ...
    if job.Resource_List["nodes"]:
        ...
        event.accept(
            "Held torque style submission (-l nodes=). Will process and release shortly."
        )
        return

    if job.Resource_List["nodect"] == 1:
        return

    nodect = int(repr(job.Resource_List["select"] or 1).split(":")[0])
    if nodect == 1:
        return

    if job.Resource_List["place"]:
        if "group=group_id" in str(job.Resource_List["place"] or ""):
            debug("Job %s explicitly set group=group_id" % job.id)
            return
    elif queue.resources_default and "group=group_id" in str(
        queue.resources_default["place"] or ""
    ):
        debug(
            "Job %s implicitly set group=group_id via queue %s's defaults"
            % (job.id, queue.name)
        )
        return
    ...
    msg.write(
        "Job uses more than one node and does not place the jobs based on group_id, which may cause issues with tightly coupled jobs.\n"
    )
    msg.write("Please do one of the following\n")
    msg.write(
        "    1) Ensure this placement is set by adding group=group_id to your -l place= statement\n"
    )
    msg.write(
        "        Note: Queue {}'s resource_defaults.place={}\n".format(
            queue.name, queue.resources_default["place"]
        )
    )
    # fmt: off
    msg.write(
    ...
    msg.write(
        "    3) Disable this hook for this queue via queue defaults -> qmgr -c 'set queue {} resources_default.skipcyclesubhook=true'\n".format(
            queue.name
        )
    )
    msg.write(
